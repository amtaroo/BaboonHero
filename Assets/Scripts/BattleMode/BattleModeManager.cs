    ...
    void SpawnTrash()
{
    if (trashPrefabs.Count > 0 && spawnPoints.Length > 0)
    {
        // Spawn first trash item
        Transform spawnPoint1 = GetAvailableSpawnPoint();
        if (spawnPoint1 != null)
        {
            GameObject trashToSpawn1 = trashPrefabs[Random.Range(0, trashPrefabs.Count)];
            Instantiate(trashToSpawn1, spawnPoint1.position, spawnPoint1.rotation);
            occupiedSpawnPoints.Add(spawnPoint1);
        }

        // Spawn second trash item
        Transform spawnPoint2 = GetAvailableSpawnPoint();
        if (spawnPoint2 != null)
        {
            GameObject trashToSpawn2 = trashPrefabs[Random.Range(0, trashPrefabs.Count)];
            Instantiate(trashToSpawn2, spawnPoint2.position, spawnPoint2.rotation);
            occupiedSpawnPoints.Add(spawnPoint2);
        }
    }
}*/
